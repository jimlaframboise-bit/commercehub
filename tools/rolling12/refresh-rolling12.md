...
The same generated page is served in two places and **both must be updated together** — a
refresh that touches one leaves the other quietly showing older numbers:

- **CommerceHub `/rolling12`** — embedded in an iframe, from `src/data/rolling12Snapshot.js`.
- **`crump-amazon-tracker.vercel.app/rolling12`** — standalone, from `rolling12/index.html` in
  the repo `jimlaframboise-bit/crump-amazon-tracker` (the same project that serves the redesign
  tracker at its root). Generated by `make-standalone12.mjs`, which only changes the `<title>`
  and adds a robots noindex tag.
...
6. `node tools/rolling12/emit-module12.mjs` — writes `src/data/rolling12Snapshot.js`.
   `node tools/rolling12/make-standalone12.mjs` — writes `tools/rolling12/standalone/index.html`
   (+ `robots.txt`) for the standalone deployment.
...
8. `node tools/rolling12/check-rolling12.mjs` — 38 assertions. Run it **twice**: once on
   `tools/rolling12/rolling12-page.html` and once on `tools/rolling12/standalone/index.html`
   (pass the path as the first argument). Both must be 38/38.
...
9. Commit and deploy **both**: `index.html` to `jimlaframboise-bit/commercehub` (root), and
   `rolling12/index.html` to `jimlaframboise-bit/crump-amazon-tracker`. Verify both live URLs
...
## The robots.txt is not access control

`robots.txt` at the root of `crump-amazon-tracker.vercel.app` disallows all crawlers, and the
standalone page carries a `noindex, nofollow` meta tag. Both only keep the page out of search
results. **Anyone with the URL can still read it, and it carries real Crump financials.** Vercel
Hobby cannot gate a production domain — see [[commercehub-tracker-page]] for the options Jim has
open. Never describe the noindex as protection.
