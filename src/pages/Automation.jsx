...
/* ============================ CAMPAIGN AI (E2 — spec §13) ============================ */
// Automated bid + budget + keyword-harvesting AI applied at the Tag or Campaign
// level. Two tabs (Tag Level / Campaign Level) mirror Pacvue's Campaign AI grid;
// the launch flow captures the core inputs (Target ROAS, Max Bid, Budget Control,
// auto-harvest). AI State is a per-row toggle persisted to localStorage.
const CAMPAI_FIELDS = [
  { key: 'name', label: 'Name', type: 'text' },
  { key: 'aiState', label: 'AI State', type: 'enum', options: ['Active', 'Paused'] },
  { key: 'market', label: 'Profile', type: 'enum', options: ['Amazon USA', 'Amazon Canada', 'Amazon UK'] },
  { key: 'budgetControl', label: 'Budget Control', type: 'enum', options: ['On', 'Off'] },
  { key: 'targetRoas', label: 'Target ROAS', type: 'number' },
  { key: 'roas30', label: 'ROAS (30d)', type: 'number' },
  ...
  return (
    <Modal title={`Launch AI for a Single ${level === 'tag' ? 'Tag' : 'Campaign'}`} sub="Set a Target ROAS and Max Bid — the AI paces bids toward it and harvests converting search terms automatically." onClose={onClose} width={480}
      ...
      <label className="fld" style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        <Toggle on={budgetOn} onClick={() => setBudgetOn((v) => !v)} /><span>Budget Control</span>
        {budgetOn && <input type="number" style={{ width: 120, marginLeft: 'auto' }} value={budgetCap} onChange={(e) => setBudgetCap(e.target.value)} placeholder="Daily cap" />}
      </label>
      <label className="fld" style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
        <Toggle on={harvest} onClick={() => setHarvest((v) => !v)} /><span>Auto keyword harvesting</span>
      </label>
      <div className="modal-note"><Icon name="bulb" size={14} /><span>Campaign AI is an automated alternative to manual rules — a good default when you want Pacvue to manage bids toward efficiency without hand-tuning.</span></div>
    </Modal>
  )
}
export function CampaignAI() {
  const { profileId } = useApp()
  const [tab, setTab] = useState('Tag Level')
  ...
  const all = useMemo(() => [...created, ...campaignAI], [created])
  const rows0 = useMemo(() => all
    .filter((r) => r.level === level)
    .filter((r) => profileId === 'all' || r.profileId === profileId)
    .map((r) => {
      const p = profileById[r.profileId] || {}
      const st = (stateOv[r.id] && stateOv[r.id].aiState) || r.aiState
      return { ...r, aiState: st, market: p.market || r.profileId, currency: p.currency || '$' }
    }), [all, level, profileId, stateOv])
  ...
  const avgTgt = filtered.length ? filtered.reduce((a, r) => a + r.targetRoas, 0) / filtered.length : 0
  const totHarvest = filtered.reduce((a, r) => a + (r.harvestKWs || 0), 0)
  const avgRoas = filtered.length ? filtered.reduce((a, r) => a + r.roas30, 0) / filtered.length : 0

  const columns = [
    { key: 'name', label: level === 'tag' ? 'Tag' : 'Campaign', sticky: true, width: 300, sortVal: (r) => r.name, render: (r) => (
      <div className="namecell"><div className="nc-top"><b>{r.name}</b>{r.level === 'tag' && <span className="tag" style={{ marginLeft: 6 }}>Tag</span>}</div><small>Target ROAS {dec2(r.targetRoas)} · Max bid {cur(r.maxBid, r.currency, 2)}{r.harvest ? ' · auto-harvest' : ''}</small></div>) },
    { key: 'aiState', label: 'AI State', sortVal: (r) => r.aiState, render: (r) => <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><Pill>{r.aiState}</Pill><Toggle on={r.aiState === 'Active'} onClick={() => toggleAI(r)} /></span> },
    { key: 'market', label: 'Profile', sortVal: (r) => r.market, render: (r) => <span>{r.market}</span> },
    { key: 'budgetControl', label: 'Budget Control', sortVal: (r) => r.budgetControl, render: (r) => r.budgetControl === 'On' ? <span className="muted">Daily {cur(r.budgetCap, r.currency)}</span> : <span className="muted">—</span> },
    { key: 'roas30', label: 'ROAS (Last 30 Days)', num: true, sortVal: (r) => r.roas30, foot: (rs) => dec2(rs.reduce((a, r) => a + r.roas30, 0) / (rs.length || 1)), render: (r) => <span style={{ color: r.roas30 >= r.targetRoas ? 'var(--green)' : r.roas30 < r.targetRoas * 0.8 ? 'var(--red)' : 'inherit', fontWeight: 600 }}>{dec2(r.roas30)}</span> },
    { key: 'harvestKWs', label: 'Harvest KWs', num: true, sortVal: (r) => r.harvestKWs, foot: (rs) => int(rs.reduce((a, r) => a + (r.harvestKWs || 0), 0)), render: (r) => int(r.harvestKWs) },
    { key: 'lastRun', label: 'Last Run', sortVal: (r) => r.lastRun, render: (r) => <span className="muted">{r.lastRun}</span> },
    { key: 'creator', label: 'Creator', sortVal: (r) => r.creator, render: (r) => <span className="muted">{r.creator}</span> },
    { key: 'createTime', label: 'Create Time', sortVal: (r) => r.createTime, render: (r) => <span className="muted">{r.createTime}</span> },
  ]
  const dims = [{ key: 'aiState', label: 'AI State' }, { key: 'budgetControl', label: 'Budget Control' }, { key: 'market', label: 'Profile' }]
  return (
    <>
      ...
      <KpiGrid>
        ...
        <Kpi label="Avg Target ROAS" value={dec2(avgTgt)} />
        <Kpi label="Avg ROAS (30d)" value={dec2(avgRoas)} deltaGood />
...
/* ============================ PRODUCT AI (E2 — spec §13) ============================ */
// Per-ASIN automation driven by a launch Strategy toward a Target ROAS. Renders
// the Managed-Events panel + KPI tiles + a performance chart above an ASIN grid
// (Weekly ROAS Trend under "Auto" bidding, Strategy, Spend/Budget, targets).
const PRODAI_FIELDS = [
  { key: 'asin', label: 'ASIN', type: 'text' },
  { key: 'title', label: 'Name', type: 'text' },
  { key: 'state', label: 'State', type: 'enum', options: ['Enabled', 'Paused'] },
  { key: 'market', label: 'Profile', type: 'enum', options: ['Amazon USA', 'Amazon Canada', 'Amazon UK'] },
  { key: 'strategy', label: 'Strategy', type: 'enum', options: ['More Conversion', 'More Traffic', 'More Efficiency'] },
  { key: 'budgetType', label: 'Budget Type', type: 'enum', options: ['Daily'] },
  { key: 'targetRoas', label: 'Target ROAS', type: 'number' },
...
const PRODAI_EVENTS = [
  ...
  { key: 'evDecBid', label: 'Decrease Bid', icon: 'trendDown', tone: 'amber' },
  { key: 'evAddTgt', label: 'Add Targeting', icon: 'target', tone: 'blue' },
  { key: 'evPauseTgt', label: 'Pause Targeting', icon: 'pause', tone: 'red' },
]
function ManagedEvents({ rows, days }) {
  const f = days / 30
  return (
    <div className="ai-events">
      {PRODAI_EVENTS.map((e) => {
        const total = Math.round(rows.reduce((a, r) => a + (r[e.key] || 0), 0) * f)
        return (
          <div key={e.key} className="ai-event">
            <div className={`ai-ev-ico ${e.tone}`}><Icon name={e.icon} size={16} /></div>
            <div><div className="ai-ev-n">{int(total)}</div><div className="ai-ev-l">{e.label}</div></div>
          </div>
        )
      })}
    </div>
...
function LaunchProductAIModal({ onClose, onLaunch }) {
  ...
  const submit = () => {
    ...
    onLaunch([{
      ...
      targets: 0, owner: 'jim@brightleaf.co',
  ...
  return (
    <Modal title="Launch a Single Product AI" sub="Choose a launch Strategy and Target ROAS — the AI manages bids and targeting for this ASIN." onClose={onClose} width={500}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <label className="fld" style={{ flex: 1 }}>Target ROAS<input type="number" step="0.1" value={targetRoas} onChange={(e) => setTargetRoas(e.target.value)} /></label>
        <label className="fld" style={{ flex: 1 }}>Daily Budget ($)<input type="number" step="50" value={dailyBudget} onChange={(e) => setDailyBudget(e.target.value)} /></label>
      </div>
      <div className="modal-note"><Icon name="bulb" size={14} /><span><b>More Conversion</b> maximizes orders at target ROAS · <b>More Traffic</b> grows impressions &amp; clicks · <b>More Efficiency</b> protects ROAS and trims wasted spend.</span></div>
    </Modal>
  )
}
export function ProductAI() {
  const { profileId, rangeResolved } = useApp()
  const [created, setCreated] = useState(() => createdStore.get('product-ai'))
  const [q, setQ] = useState('')
  ...
  const mixed = new Set(filtered.map((r) => r.profileId)).size > 1
  const note = mixed ? ' (USD est.)' : ''
  const dispSym = mixed ? '$' : (filtered[0] ? filtered[0].currency : '$')
  const toUsd = (r) => (mixed ? { ...r, spend: (r.spend || 0) * fxUSD(r.profileId), sales: (r.sales || 0) * fxUSD(r.profileId) } : r)
  const moneyFoot = (key) => (rs) => cur(rs.reduce((a, r) => a + (r[key] || 0) * (mixed ? fxUSD(r.profileId) : 1), 0), dispSym) + note
  ...
  const columns = [
    { key: 'asin', label: 'ASIN', sticky: true, width: 300, sortVal: (r) => r.title, render: (r) => (
      <div className="namecell"><div className="nc-top"><b>{(r.title || '').split('—')[0].trim()}</b></div><small>ASIN {r.asin} · {r.market}</small></div>) },
    { key: 'state', label: 'State', sortVal: (r) => r.state, render: (r) => <Pill>{r.state}</Pill> },
    { key: 'trend', label: 'Weekly ROAS Trend', sort: false, width: 170, render: (r) => (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span className="tag" style={{ color: 'var(--brand)' }}>{r.bidMode}</span><div style={{ width: 92 }}><MiniLine data={r.trend} dataKey="roas" color="#6b4fd6" height={28} /></div></div>) },
    { key: 'strategy', label: 'Strategy', sortVal: (r) => r.strategy, render: (r) => <Pill tone="blue">{r.strategy}</Pill> },
    { key: 'spendBudget', label: 'Spend/Budget', num: true, sort: false, render: (r) => { const pc = r.dailyBudget ? (r.dailySpend / r.dailyBudget) * 100 : 0; return <div style={{ textAlign: 'right' }}><b>{Math.round(pc)}%</b><div className="muted" style={{ fontSize: 11 }}>{r.budgetType}: {cur(r.dailySpend, r.currency)} / {cur(r.dailyBudget, r.currency)}</div></div> } },
    { key: 'targets', label: 'Targets', num: true, sortVal: (r) => r.targets, foot: (rs) => int(rs.reduce((a, r) => a + (r.targets || 0), 0)), render: (r) => <span className="muted">{int(r.targets)} targets</span> },
    { key: 'lastRunEnd', label: 'Last Run', sort: false, render: (r) => <span className="muted" style={{ fontSize: 11 }}>{r.lastRunStart} ~ {r.lastRunEnd}</span> },
    { key: 'owner', label: 'Owner', sortVal: (r) => r.owner, render: (r) => <span className="muted">{r.owner}</span> },
    { key: 'targetRoas', label: 'Target ROAS', num: true, sortVal: (r) => r.targetRoas, render: (r) => dec2(r.targetRoas) },
    { key: 'sales', label: 'Sales', num: true, foot: moneyFoot('sales'), render: (r) => cur(r.sales, r.currency) },
    { key: 'spend', label: 'Spend', num: true, foot: moneyFoot('spend'), render: (r) => cur(r.spend, r.currency) },
    { key: 'roas', label: 'ROAS', num: true, foot: (rs) => dec2(aggregate(rs.map(toUsd)).roas), render: (r) => dec2(r.roas) },
    { key: 'acos', label: 'ACoS', num: true, foot: (rs) => pct(aggregate(rs.map(toUsd)).acos), render: (r) => <span style={{ color: r.acos > 40 ? 'var(--red)' : r.acos < 25 ? 'var(--green)' : 'inherit', fontWeight: 600 }}>{pct(r.acos)}</span> },
  ...
  const presets = {
    'Default Columns': ['asin', 'state', 'trend', 'strategy', 'spendBudget', 'targets', 'lastRunEnd', 'owner', 'sales', 'spend', 'roas', 'acos'],
    Performance: ['asin', 'strategy', 'sales', 'spend', 'roas', 'acos', 'impr', 'clk', 'cvr', 'orders'],
  }
  const dims = [{ key: 'strategy', label: 'Strategy' }, { key: 'state', label: 'State' }, { key: 'market', label: 'Profile' }]
  return (
    <>
      ...
      <div className="ai-dash">
        <Card title="Managed Events" sub={`AI actions · ${rangeResolved.label}`} pad={false}>
          <ManagedEvents rows={filtered} days={rangeResolved.days} />
        </Card>
        <KpiGrid>
          <Kpi label="Impressions" value={compact(agg.impr)} />
          <Kpi label="Clicks" value={compact(agg.clk)} />
          <Kpi label="Orders" value={compact(agg.orders)} />
          <Kpi label="Spend" value={money(agg.spend, dispSym) + note} />
          <Kpi label="Sales" value={money(agg.sales, dispSym) + note} />
          <Kpi label="ACoS" value={pct(agg.acos)} />
          <Kpi label="ROAS" value={dec2(agg.roas)} />
          <Kpi label="CVR" value={pct(agg.cvr)} />
        </KpiGrid>
      </div>
